...
public class BluetoothCrashResolver {
  ...
  public void notifyScannedDevice(BluetoothDevice device, BluetoothAdapter.LeScanCallback scanner) {
    ...
    if (distinctBluetoothAddresses.size() > getCrashRiskDeviceCount()) {
      if (PREEMPTIVE_ACTION_ENABLED && !recoveryInProgress) {
        Log.w(TAG, "Large number of bluetooth devices detected: "
            + distinctBluetoothAddresses.size()
            + " Proactively attempting to clear out address list to prevent a crash");
  ...
  public void crashDetected() {
    ...
    if (distinctBluetoothAddresses.size() > 0) {
      if (isDebugEnabled()) {
        Log.d(TAG,
            "Distinct bluetooth devices seen at crash: " + distinctBluetoothAddresses.size());
      }
    }
  ...
  private void startRecovery() {
    ...
    if (!adapter.isDiscovering()) {
      ...
      if (TIME_TO_LET_DISCOVERY_RUN_MILLIS > 0) {
        if (isDebugEnabled()) {
          Log.d(TAG, "We will be cancelling this discovery in "
      ...
      if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
        ...
        switch (state) {
          ...
          case BluetoothAdapter.STATE_ON:
            ...
            if (isDebugEnabled()) {
              Log.d(TAG, "Bluetooth was turned off for " + (lastBluetoothTurningOnTime
                  - lastBluetoothOffTime) + " milliseconds");
            }
