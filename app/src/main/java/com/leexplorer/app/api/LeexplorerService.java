...
public interface LeexplorerService {
  ...
  @GET("/gallery") List<Gallery> getGalleries();

  @GET("/gallery/{id}") Gallery getGallery(@Path("id") String galleryId);

  @POST("/gallery_list") List<Gallery> getGalleryList(@Body List<String> galleryIds);

  @POST("/artwork/like/{value}") void likeArtwork(@Path("value") int value, Callback<Void> cb);
}
